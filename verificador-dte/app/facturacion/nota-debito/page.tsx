...
export default function NotaDebitoPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [emitterRes, receptorsRes, res] = await Promise.all([
        ...
        fetch('/api/facturacion/debit-note-related?limit=100', {
      ...
      setError(err instanceof Error ? err.message : 'No se pudo cargar nota de debito');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submitNote() {
    ...
    try {
      ...
      const token = await firebaseToken();
      const res = await fetch('/api/facturacion/debit-notes', {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(payload.error || 'No se pudo emitir nota de debito');
      setResult(payload);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo emitir nota de debito');
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <section className="space-y-4">
        <div className="rounded-lg border border-border bg-card p-5 shadow-sm">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight">Nota de debito</h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
                Documento tipo 06 para aumentar o cargar diferencias sobre un comprobante de credito fiscal emitido al mismo receptor.
              </p>
            </div>
          ...
          <>
                ...
                <CardHeader className="pb-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <CardDescription>Datos oficiales del emisor y receptor de la nota de debito.</CardDescription>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Selecciona el credito fiscal que sera aumentado o ajustado con este cargo.</CardDescription>
              </CardHeader>
              ...
              <CardHeader>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>Se cargan desde el CCF. Puedes ajustar cantidades o montos segun el cargo adicional.</CardDescription>
                  </div>
                ...
                <div className="grid gap-3 pt-3 lg:grid-cols-[minmax(0,1fr)_34rem]">
                  ...
                  <div>
                    ...
                    <div className="overflow-hidden rounded-lg border border-border text-sm">
                      ...
                      <SummaryRow label="Total a Debitar" value={money(summary.totalPagar)} strong />
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            {result && <p className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">Nota de debito generada: <span className="font-mono">{result.codigoGeneracion}</span></p>}
          </>
        )}
      </section>

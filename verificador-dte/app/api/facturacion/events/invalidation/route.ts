...
type EventType = 'invalidation' | 'contingency' | 'special-operations' | 'return';

const ROUTES: Record<EventType, string> = {
  invalidation: '/api/facturacion/events/invalidation/submit',
  contingency: '/api/facturacion/events/contingency/submit',
  'special-operations': '/api/facturacion/events/special-operations/submit',
  return: '/api/facturacion/events/return/submit',
};

async function handleEvent(req: NextRequest, eventType: EventType) {
  try {
    ...
    const response = await postGoFacturacion(ROUTES[eventType], {
      ...body,
      environment,
      passwordPri,
...
export async function POST(req: NextRequest) {
  return handleEvent(req, 'invalidation');
}

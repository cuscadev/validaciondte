...
export async function POST(req: NextRequest) {
  ...
  if (!email || !password || !licenseKey || !deviceId || !deviceName) {
    return NextResponse.json(
      { error: 'email, password, licenseKey, deviceId y deviceName son requeridos' },
      { status: 400 }
    );
  }
  ...
  await deviceRef.set(
    {
      ...
      lastSeen: new Date(),
      updatedAt: new Date(),
      createdAt: deviceSnap.exists ? deviceSnap.data()?.createdAt || new Date() : new Date(),
    },
    { merge: true }
  );
  ...
  await licenseRef.set(
    {
      ...
      userEmail: email,
      updatedAt: new Date(),
      allowedDevices: Array.from(new Set([...(Array.isArray(license?.allowedDevices) ? license.allowedDevices : []), deviceId])),
    },
    { merge: true }
  );

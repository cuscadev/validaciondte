...
import { getSyncJob, listDocuments, listSyncJobResults } from '@/lib/email/db';
...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const results = await listSyncJobResults(jobId, user.organizationId);

    return NextResponse.json({ job, documents, results, total });
  } catch (error) {

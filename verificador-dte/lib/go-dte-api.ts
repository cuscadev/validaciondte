...
export type StartEmailSyncInput = {
  jobId: string;
...
export async function startEmailSyncViaGo(input: StartEmailSyncInput): Promise<void> {
  const apiKey = process.env.GO_DTE_INTERNAL_API_KEY?.trim();
  if (!apiKey) {
    throw new Error('Falta GO_DTE_INTERNAL_API_KEY en el servidor.');
  }

  const res = await fetch(`${getGoDteApiUrl()}/api/email/sync/run`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(text || `Go email sync error ${res.status}`);
  }
}
